...
namespace knearestTest
{
    ...
    public class KnnTest
    {
        ...
        {
            Matrix queryMatrix = DenseMatrix.CreateRandom(3, 100, new ContinuousUniform(-10, 10));
            DenseColumnMajorMatrixStorage<float> query = (DenseColumnMajorMatrixStorage<float>)queryMatrix.Storage;
            Matrix points = DenseMatrix.CreateRandom(3, 100, new ContinuousUniform(-10, 10));
            Vector maxRadii = DenseVector.Create(100, i => float.PositiveInfinity);
